 ...
 *
 * This file was originally part of the MIPL Mobile IPv6 for Linux.
 * and has been adapted for LinShim6 by S. Barré.
 * 
 * Authors:
 *  Ville Nuorvala <vnuorval@tcs.hut.fi>,
 *  Antti Tuominen <anttit@tcs.hut.fi>
 *
 * Modified by Sébastien Barré. (February 2008)
 *
...
/**
 * route_get - Get a route towards a given destination
 * @src: Filled by the caller. Source address.
 * @dst: Filled by the caller, Destination address.
 * @oif: Filled by this function, output interface.
 * @gateway: Filled by this function, gateway.
 * @metrics: Filled by this function, metrics.
 *
 */
	      ...
	      struct in6_addr *gateway, int* metrics)
{
	...
	sn->nlmsg_len = NLMSG_LENGTH(sizeof(struct rtmsg));
	sn->nlmsg_flags = NLM_F_REQUEST;

	sn->nlmsg_type = RTM_GETROUTE;

	rtm = NLMSG_DATA(sn);
	rtm->rtm_family = AF_INET6;

	addattr_l(sn, sizeof(sbuf), RTA_DST, dst, sizeof(*dst));
	addattr_l(sn, sizeof(sbuf), RTA_SRC, src, sizeof(*src));
	
	memset(rbuf, 0, sizeof(rbuf));
	rn = (struct nlmsghdr *)rbuf;

        if ((err=rtnl_route_do(sn, rn))<0) return err;
	
	if (rn->nlmsg_type != RTM_NEWROUTE || 
	    rn->nlmsg_len < NLMSG_LENGTH(sizeof(*rtm)))
		return -EINVAL;
	...
	memset(rta_tb, 0, sizeof(rta_tb));
	parse_rtattr(rta_tb, RTA_MAX, RTM_RTA(rtm), 
		     rn->nlmsg_len - NLMSG_LENGTH(sizeof(*rtm)));
	
	if (!rta_tb[RTA_OIF] || !rta_tb[RTA_METRICS]) return -EINVAL;

	*oif=*(int*)RTA_DATA(rta_tb[RTA_OIF]);
	*metrics=*(int*)RTA_DATA(rta_tb[RTA_METRICS]);
	if (rta_tb[RTA_GATEWAY])
		memcpy(gateway,(struct in6_addr*)
		       RTA_DATA(rta_tb[RTA_GATEWAY]),sizeof(struct in6_addr));
	else bzero(gateway,sizeof(struct in6_addr));
